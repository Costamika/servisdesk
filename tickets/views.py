...
from .forms import TicketForm, TicketAdminForm, TicketCommentForm, TicketSearchForm
...
def ticket_detail(request: HttpRequest, ticket_id: int) -> HttpResponse:
    ...
    context = {
        ...
        'is_admin': is_admin(request.user),
        'user_list': User.objects.filter(is_active=True).order_by('first_name', 'last_name') if is_admin(request.user) else [],
    }
...
def ticket_edit(request: HttpRequest, ticket_id: int) -> HttpResponse:
    ...
    if not (request.user == ticket.created_by or request.user.is_staff or request.user.is_superuser or request.user == ticket.assigned_to):
...
def ticket_assign(request: HttpRequest, ticket_id: int) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if assigned_to_id:
            ...
            ticket.save()
            messages.success(request, f'Заявка назначена пользователю {assigned_to.get_full_name()}')
        else:
...
def ticket_change_status(request: HttpRequest, ticket_id: int) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if new_status in dict(Ticket.STATUS_CHOICES):
            ...
            ticket.save()
            messages.success(request, f'Статус заявки изменен на "{dict(Ticket.STATUS_CHOICES)[new_status]}"')
